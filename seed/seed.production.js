...
fs.readdir(path.join(__dirname, './currentData/housesData'), (err, files) => {
  if (err) throw err;
  exec(`mongoimport -h ${SOURCE} -d housing-heatmap -c postcodes --type csv --file ./seed/currentData/ukpostcodes.csv --headerline -u ${USERNAME} -p ${PASSWORD}`, err => {
    if (err) throw err;
    console.log('Postcodes data added!');
    files.map(file => {
